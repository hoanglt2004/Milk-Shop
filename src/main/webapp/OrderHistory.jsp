                        ...
                        <div class="content-body">
                            ...
                            <div class="stats-cards">
                                ...
                                <div class="stat-card">
                                    ...
                                    <div class="value">
                                        <fmt:formatNumber value="${totalSpent}" pattern="#,###" var="formattedTotal" />
                                ...
                                <form method="get" action="orderHistory">
                                    <div class="filter-row">
                                        ...
                                        <div class="filter-group">
                                            ...
                                            <select name="time">
                                                <option value="" ${empty timeFilter ? 'selected' : ''}>Tất cả</option>
                                                <option value="30days" ${timeFilter == '30days' ? 'selected' : ''}>30 ngày qua</option>
                                                <option value="3months" ${timeFilter == '3months' ? 'selected' : ''}>3 tháng qua</option>
                                                <option value="6months" ${timeFilter == '6months' ? 'selected' : ''}>6 tháng qua</option>
                                                <option value="1year" ${timeFilter == '1year' ? 'selected' : ''}>1 năm qua</option>
                                            </select>
                                        </div>
                                        <div class="filter-group">
                                            <label>Tìm kiếm</label>
                                            <input type="text" name="search" placeholder="Mã đơn hàng..." value="${searchKeyword}">
                                        </div>
                                        <div class="filter-group">
                                    ...
                                    <c:forEach var="invoice" items="${invoiceList}">
                                        <div class="order-card">
                                            <div class="order-header">
                                                <div>
                                                    <div class="order-id">Đơn hàng #${invoice.maHD}</div>
                                                ...
                                                <div>
                                                    <c:choose>
                                                        <c:when test="${invoice.status == 'Chờ xác nhận'}">
                                                            <span class="status-badge status-pending">${invoice.status}</span>
                                                        </c:when>
                                                        <c:when test="${invoice.status == 'Đang giao'}">
                                                            <span class="status-badge status-processing">${invoice.status}</span>
                                                        </c:when>
                                                        <c:when test="${invoice.status == 'Hoàn thành'}">
                                                            <span class="status-badge status-completed">${invoice.status}</span>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <span class="status-badge status-cancelled">${invoice.status}</span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </div>
                                            </div>
                                            ...
                                            <div class="order-body">
                                                <div class="order-info">
                                                    <div class="info-item">
                                                        ...
                                                        <div class="info-value">
                                                            <fmt:formatNumber value="${invoice.tongGia}" pattern="#,###" var="orderTotal" />
                                                    ...
                                                    <c:if test="${not empty invoice.deliveryDate}">
                                                        <div class="info-item">
                                                            ...
                                                            <div class="info-value">
                                                                <fmt:formatDate value="${invoice.deliveryDate}" pattern="dd/MM/yyyy" />
                                                            </div>
                                                        </div>
                                                    </c:if>
                                                </div>
                                                ...
                                                <div class="order-actions">
                                                    <button class="btn-action btn-view" onclick="viewOrderDetail(${invoice.maHD})">
                                                    ...
                                                    <c:if test="${invoice.status == 'Chờ xác nhận'}">
                                                        <button class="btn-action btn-cancel" onclick="cancelOrder(${invoice.maHD})">
